...
- (instancetype)initWithApplication:(Application *)application
{
    ...
    if (self) {
//        self.application = [[self filteredApps] objectAtIndex:1];
...
//    self.application.displayName  = @"airbnb";
//    self.application.developerName = @"Airbnb, Inc.";
//    self.application.identifier = @"401626263";
//    self.application.iconName = @"icon_airbnb";
//    self.application.type = ApplicationTypeAirbnb;
    [self serializeApplications];
    
    self.application = [[self filteredApps] objectAtIndex:1];
    self.allowShuffling = YES;
...
//    cell.textLabel.text = object[@"users"];
//    
//    return cell;
//}
...
{
    static NSString *identifier = @"MatchedCell";
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (!cell) {
        cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:identifier];
    }
//    cell.textLabel.text = [[[self.objects objectAtIndex:indexPath.row] objectForKey:@"users"] objectAtIndex:0];
    cell.textLabel.text = @"Someone matched";
    NSString *time = [timeFormatter stringForTimeIntervalFromDate:[NSDate date] toDate:[[self.objects objectAtIndex:indexPath.row] createdAt]];
...
- (PFQuery *)queryForTable {
    ...
    PFQuery *query = [PFQuery queryWithClassName:self.parseClassName];
    NSArray *userArray = [[NSArray alloc] initWithObjects:[[PFUser currentUser] objectForKey:@"botId"], nil];
    [query whereKey:@"users" containsAllObjectsInArray:userArray];
...
- (void)objectsDidLoad:(NSError *)error
{
    ...
    NSLog(@"objects did load error:%@", error);
}
