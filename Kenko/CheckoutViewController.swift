...
class CheckoutViewController: UIViewController, STPPaymentContextDelegate {
    ...
    let stripePublishableKey = "pk_test_oCaXRnNN88e5zk5A5BCzkY5z"
    
    // 2) Next, optionally, to have this demo save your user's payment details, head to
    // https://github.com/stripe/example-ios-backend , click "Deploy to Heroku", and follow
    // the instructions (don't worry, it's free). Replace nil on the line below with your
    // Heroku URL (it looks like https://blazing-sunrise-1234.herokuapp.com ).
    let backendBaseURL: String? = "https://kenkotoday.herokuapp.com"

    // 3) Optionally, to enable Apple Pay, follow the instructions at https://stripe.com/docs/mobile/apple-pay
    // to create an Apple Merchant ID. Replace nil on the line below with it (it looks like merchant.com.yourappname).
    let appleMerchantID: String? = "merchant.tw.today.kenko"
    ...
    func paymentContext(paymentContext: STPPaymentContext, didFinishWithStatus status: STPPaymentStatus, error: NSError?) {
        ...
        let action = UIAlertAction(title: "Ok", style: .Default) { (action) in
            ...
            let buyActivity = PFObject(className: kPAPActivityClassKey)
            buyActivity.setObject(kPAPActivityTypeBuyKey, forKey: kPAPActivityTypeKey)
            buyActivity.setObject(PFUser.currentUser()!, forKey: kPAPActivityFromUserKey)
            buyActivity.setObject(self.product, forKey: kPAPActivityEmojiKey)
            
            let likeACL = PFACL(user: PFUser.currentUser()!)
            //            likeACL.setPublicReadAccess(true)
            likeACL.setWriteAccess(true, forUser: PFUser.currentUser()! )
            buyActivity.ACL = likeACL
            
            buyActivity.saveInBackgroundWithBlock { (succeeded, error) in
                if succeeded {
                    let user = PFUser.currentUser()
                    user?.setObject(self.product, forKey: kPAPActivityEmojiKey)
                    user?.saveEventually({ (succeeded, error) in
